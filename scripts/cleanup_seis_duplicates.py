...
크롤러 수리(커밋 6cfed6d)는 **앞으로 들어오는** 데이터만 바로잡는다.
이미 저장된 행은 재크롤로 사라지지 않으므로 이 스크립트가 치운다.

정리 대상 3종:

1. **seis 복수 링크 중복** - SEIS 메인이 같은 공고를 회차별 링크로 나열해
   동일 공고가 9행까지 적재됐다. 대표 1행만 남긴다.
...
def canonical_rank(row: sqlite3.Row) -> Tuple[str, int, int]:
    """대표 행 선정 순위 - 접수 시작일이 늦고 공고번호가 큰 쪽이 이긴다."""
    ...
    return (row["period_start"] or "", round_no, post_no)


def find_seis_duplicates(conn: sqlite3.Connection) -> Tuple[List[sqlite3.Row], List[str]]:
    """삭제할 seis 행과 판단 근거를 찾는다.
    ...
    """
    rows = conn.execute(
        "SELECT id, source_id, title, url, period_start, period_end,"
        "       raw_data, created_at,"
        "       strftime('%Y-%W', created_at) AS week"
        " FROM announcements WHERE source = 'seis'"
    ).fetchall()
    ...
    for row in rows:
        ...
        for source_id in merged:
            victim = by_source_id.get(str(source_id))
    ...
    for row in rows:
        groups.setdefault((normalize_title(row["title"]), row["week"] or ""), []).append(row)

    for (title_key, week), group in groups.items():
        if len(group) < 2:
            continue
...
def find_fake_deadlines(conn: sqlite3.Connection) -> Tuple[List[sqlite3.Row], List[str]]:
...
def find_fake_starts(conn: sqlite3.Connection) -> Tuple[List[sqlite3.Row], List[str]]:
...
def counts(conn: sqlite3.Connection) -> Dict[str, int]:
    ...
    return {
        "seis_rows": one("SELECT COUNT(*) FROM announcements WHERE source='seis'"),
        "seis_distinct_titles": one(
            "SELECT COUNT(DISTINCT title) FROM announcements WHERE source='seis'"
        ),
        "se_rows_with_deadline": one(
            "SELECT COUNT(*) FROM announcements WHERE source='socialenterprise'"
            " AND period_end IS NOT NULL AND period_end != ''"
        ),
        "coop_rows_with_start": one(
            "SELECT COUNT(*) FROM announcements WHERE source='coop'"
            " AND period_start IS NOT NULL AND period_start != ''"
        ),
        "total_rows": one("SELECT COUNT(*) FROM announcements"),
    }
...
def main() -> int:
    ...
    # (b) seis 복수 링크 중복
    seis_doomed, seis_reasons = find_seis_duplicates(conn)
    print(f"\n[b] seis 중복 삭제 대상: {len(seis_doomed)} 행")
    for line in seis_reasons:
        print(line)

    # (c) socialenterprise 가짜 마감
    fake_deadlines, deadline_reasons = find_fake_deadlines(conn)
    print(f"\n[c] socialenterprise period_end -> NULL: {len(fake_deadlines)} 행")
    for line in deadline_reasons:
        print(line)

    # (c-확장) coop 가짜 접수 시작일
    fake_starts, start_reasons = find_fake_starts(conn)
    ...
    if apply_changes:
        ...
        projected["seis_rows"] -= len(seis_doomed)
        projected["se_rows_with_deadline"] -= len(fake_deadlines)
        projected["coop_rows_with_start"] -= len(fake_starts)
        projected["total_rows"] -= len(seis_doomed)
