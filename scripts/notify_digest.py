...
def main():
    ...
    # H5 (라운드 2, Codex MEDIUM): 만료·회수는 **새 미리보기가 실제로 게시된 것을
    # 확인한 뒤에만** 한다. 조건 셋을 모두 본다:
    #   · `prepared` — 차단 안내가 아니라 **승인 가능한 미리보기**였다
    #   · `delivered_all` — 조각이 전부, 중복 없이 도착했다
    #   · `rc == 0` — 상태 기록(잠금 ②)까지 끝났다 = 그 승인 세대가 살아 있다
    # 하나라도 아니면 옛 호는 **그대로 유효**하다. 편집자에게 최소한 옛 미리보기는
    # 남아 있어야 한다(라운드 1 은 조립 성공만 보고 미리 만료시켰다).
    if rc == 0 and prepared is not None and delivered_all and message_ids:
        expire_stale_issues(markdown_path.parent, week)
        drain_expired_queues(token, chat_id, markdown_path.parent, week)
    else:
        _out("옛 호 만료·회수 보류 — 새 미리보기 게시가 확인되지 않았습니다")
    return rc


# 만료를 잠금 경합으로 건너뛴 호를 적어 두는 자리 (라운드 2, Codex MEDIUM).
# 파일 하나이고 호 잠금과 무관하다 — 다음 회차가 읽고 다시 시도한다.
EXPIRE_PENDING_NAME = ".expire_pending"


def expire_pending_path(out_dir) -> Path:
    return Path(out_dir) / EXPIRE_PENDING_NAME


def read_expire_pending(out_dir) -> list:
    """만료가 밀린 호 목록 (없거나 손상이면 빈 목록)."""
    path = expire_pending_path(out_dir)
    try:
        data = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, ValueError):
        return []
    return [key for key in data if state_mod.valid_issue(key)] \
        if isinstance(data, list) else []


def write_expire_pending(out_dir, keys) -> None:
    """밀린 목록을 갱신한다. 비면 파일을 지운다(잠금 파일과 달리 좌표가 아니다)."""
    path = expire_pending_path(out_dir)
    ...
    try:
        ...
        _err(f"⚠️  만료 보류 목록 기록 실패(진행함): {redact(exc)}")


def expire_stale_issues(out_dir, week) -> list:
    """H5: 새 호가 게시됐으니 **더 오래된 열린 호**를 만료시킨다.
    ...
    (``superseded_message_ids``)로 옮긴다. 회수(삭제)는 큐 소비 경로가 한다.

    안전 규율:
      · 주간호에서만 돈다 (월간호는 자기 계열을 따로 가진다).
      · 잠금은 **비차단**이다 — 다른 작성자가 그 호를 쥐고 있으면 건너뛰고
        ``.expire_pending`` 에 적는다. 잠금을 기다리면 두 호의 잠금이 겹쳐
        교착이 가능해진다. 밀린 호는 **다음 회차가 다시 시도**한다(라운드 2).
      · 어떤 실패도 이 잡을 실패시키지 않는다 (경고 한 줄).
    ...
    for path in candidates:
        ...
        try:
            ...
            pending.append(key)
            _err(f"⚠️  {key} 만료 보류(다른 작성자) — 다음 회차에 다시 시도: "
                 f"{redact(exc)}")
            continue
        try:
            ...
            pending.append(key)
            _err(f"⚠️  {key} 만료 실패(다음 회차 재시도): {redact(exc)}")
            continue
        finally:
            state_mod.release_lock(handle)
        expired.append(key)
        _out(f"✓ 옛 호 만료: {key} — 미리보기·안내를 회수 큐로 넘겼습니다")
    write_expire_pending(out_dir, pending)
    if pending:
        _err(f"⚠️  만료 보류 {len(pending)}건: {', '.join(pending)}")
    return expired
