"""행정안전부 사회연대경제 크롤러 - 행정안전부 사회연대경제 지원사업 공고 수집"""
import hashlib
import json
import re
...
class MoisSseCrawler(BaseCrawler):
    """행정안전부 사회연대경제(mois.go.kr) 공고 게시판 크롤러.

    대상 URL:
        - https://www.mois.go.kr/frt/bbs/type001/commonSelectBoardList.do?bbsId=BBSMSTR_000000000062
    ...
    BOARD_PATHS = ["/frt/bbs/type001/commonSelectBoardList.do?bbsId=BBSMSTR_000000000062"]

    def __init__(self):
        super().__init__(source_name="mois_sse")
    ...
    def fetch(self) -> List[RawAnnouncement]:
        """행정안전부 사회연대경제 공고 게시판에서 공고를 수집한다."""
        if BeautifulSoup is None:
        ...
        for board_path in self.BOARD_PATHS:
            ...
            self.logger.info(f"Fetching from MOIS SSE announcement board: {url}")
