"""경기환경에너지진흥원 크롤러 - 경기환경에너지진흥원 지원사업 공고 수집"""
import hashlib
import json
import re
...
class GgeeaCrawler(BaseCrawler):
    ...
    def fetch(self) -> List[RawAnnouncement]:
        ...
        for board_path in self.BOARD_PATHS:
            ...
            self.logger.info(f"Fetching from Ggeea announcement board: {url}")
