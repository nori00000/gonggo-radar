"""경기도경제과학진흥원 크롤러 - 공지사항/지원사업 공고 수집"""
import hashlib
import json
import re
...
class GbsaCrawler(BaseCrawler):
    """경기도경제과학진흥원(gbsa.or.kr) 공고 게시판 크롤러.

    대상 URL:
        - https://www.gbsa.or.kr/board/notice.do
    ...
    BOARD_PATHS = ["/board/notice.do"]

    def __init__(self):
        super().__init__(source_name="gbsa")
    ...
    def fetch(self) -> List[RawAnnouncement]:
        ...
        for board_path in self.BOARD_PATHS:
            ...
            self.logger.info(f"Fetching from GBSA announcement board: {url}")
    ...
    def _extract_post_id(self, link: str) -> str:
        ...
        id_params = [
            r"seq=(\d+)", r"announcementId=(\d+)", r"notifyId=(\d+)",
            r"nttId=(\d+)", r"idx=(\d+)", r"no=(\d+)",
