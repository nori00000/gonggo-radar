...
class IpetCrawler(BaseCrawler):
    """농림식품기술기획평가원(ipet.re.kr) 공고 게시판 크롤러.

    대상 URL:
        - https://www.ipet.re.kr/index.do?boardId=notice
    ...
    # P2-X (감사 V §4): ReadTimeout 은 TLS 와 무관한 **서버 지연**이다.
    # 실측 2026-09-15 (12회 GET): 성공 3회의 응답 시간은 0.11·0.12·0.98초,
    # 실패 9회는 전부 읽기 타임아웃 만료. 타임아웃 45초로도 실패했다 —
    # **오면 1초 안에 오고, 안 오면 오래 기다려도 안 온다.**
    # 그래서 "길게 3번" 대신 "짧게 4번" 이다. 정책 비교(2회 독립 표본, 각 5반복,
    # 같은 URL, 2026-09-15 16:4x~16:5x KST):
    #   옛 3×30s/지연5s    → 성공 4/5·3/5 (합 7/10), 합계 275.9s·270.7s, 최악 100.2s
    #   새 4×(5,8)s/지연2s → 성공 4/5·5/5 (합 9/10), 합계  47.8s· 49.9s, 최악  26.2s
    # 같은 소스에서 성공률은 떨어지지 않고 런 시간은 약 5.6배 줄었다.
    # (표본이 작다 — 주장하는 것은 "더 빠르고 나쁘지 않다" 까지다.)
    CONNECT_TIMEOUT = 5.0
    READ_TIMEOUT = 8.0
    RETRY_COUNT = 4
    RETRY_DELAY = 2.0
    ...
    def fetch(self) -> List[RawAnnouncement]:
        ...
        for board_path in self.BOARD_PATHS:
            ...
            self.logger.info(f"Fetching from IPET board: {url}")
