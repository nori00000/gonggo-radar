...
class GoyangStartupCrawler(BaseCrawler):
    ...
    def fetch(self) -> List[RawAnnouncement]:
        ...
        for board_path in self.BOARD_PATHS:
            ...
            self.logger.info(f"Fetching from Goyang Startup announcement board: {url}")
    ...
    def _to_announcement(self, item: dict, base_url: str) -> Optional[RawAnnouncement]:
        ...
        try:
            ...
            return RawAnnouncement(
                source="goyang_startup",
                source_id=source_id,
                title=title,
                url=link,
                summary="",
                author=author or "고양스타트업",
                category=category,
                target="",
