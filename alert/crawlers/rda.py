...
class RdaCrawler(BaseCrawler):
    """농촌진흥청(rda.go.kr) 공고 게시판 크롤러.

    대상 URL:
        - https://www.rda.go.kr/board/board.do?prgId=nei_ancmttEntry&menu_id=nei
    ...
    BOARD_PATHS = ["/board/board.do?prgId=nei_ancmttEntry&menu_id=nei"]

    def __init__(self):
        super().__init__(source_name="rda")
    ...
    def fetch(self) -> List[RawAnnouncement]:
        ...
        for board_path in self.BOARD_PATHS:
            ...
            self.logger.info(f"Fetching from RDA board: {url}")
    ...
    def _parse_generic_links(self, soup: "BeautifulSoup") -> List[dict]:
        ...
        view_patterns = [
            re.compile(r"mode=view", re.I),
            re.compile(r"dataNo=", re.I),
            re.compile(r"board\.do", re.I),
        ]
    ...
    def _extract_post_id(self, link: str) -> str:
        ...
        id_params = [
            ...
            r"articleId=(\d+)", r"artclId=(\d+)", r"dataNo=(\d+)",
        ]
