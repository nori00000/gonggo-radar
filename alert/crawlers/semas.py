...
class SemasCrawler(BaseCrawler):
    """소상공인시장진흥공단(semas.or.kr) 공고 게시판 크롤러.

    대상 URL:
        - https://www.semas.or.kr (메인 페이지에서 공고 링크 추출)
    ...
    def fetch(self) -> List[RawAnnouncement]:
        ...
        if self.BOARD_PATHS:
            for board_path in self.BOARD_PATHS:
                ...
                self.logger.info(f"Fetching from SEMAS board: {url}")
