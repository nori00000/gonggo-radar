...
자기 파서로 항목을 세면 게이트의 계수와 조용히 갈라진다.
"""
...
def item_urls(markdown_text: str) -> List[str]:
    ...
    return blocks_mod.item_urls(markdown_text)
...
def parse_digest(markdown_text: str) -> Dict:
    ...
    for block in blocks_mod.parse_blocks(markdown_text):
        ...
        if block["kind"] == "section":
            ...
            heading = block["name"]
            current = {
                "name": HEADING_TO_SECTION.get(heading, block["key"]),
                "heading": heading,
                "items": [],
            }
...
def render_preview(
    ...
    document = blocks_mod.parse_blocks(markdown_text)
